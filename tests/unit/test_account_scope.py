...
def test_resolve_sub_account_from_scope_main_uses_default() -> None:
    assert (
        resolve_sub_account_from_scope(account="main", default="sub-123") == "sub-123"
    )
...
def test_resolve_sub_account_from_scope_dict() -> None:
    assert resolve_sub_account_from_scope(
        account={"subaccountId": "abc"}
    ) == "abc"
